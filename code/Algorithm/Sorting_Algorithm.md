...
5. left 인덱스의 값과 right 인덱스의 값을 바꿈

6. 3, 4, 5 과정을 left<right 가 만족될 때 까지 반복

7. 위 과정이 끝나면 left의 값과 pivot point를 바꿔준다

8. 맨 왼쪽부터 left -1까지, left+1부터 맨 오른쪽 까지로 나눠 퀵 정렬을 반복

퀵 정렬은 분할과 동시에 정렬을 진행하는 알고리즘

각 정렬은 배열의 크기 N만큼 비교하며, 이를 총 분할 깊이인 logN만큼 진행하므로 총 비교횟수는 NlogN, 즉 시간복잡도는 **O(NlogN)**

**다만 최악의 경우가 존재하는데, 이는 배열이 이미 정렬되어있는 경우**

**이 경우 분할이 N 만큼 일어나므로 시간 복잡도는 O(N^2)이다**

이를 방지하기 위해 전체 배열 값 중 중간값이나 랜덤 값으로 pivot point를 정하는 방법을 쓰기도 함

**평균적으로 O(logN) 의 공간복잡도, 최악의 경우 O(N)의 공간 복잡도**
